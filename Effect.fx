...
struct VS__TRANSLATE_TEXTURED_LIGHTING_COLOR_INPUT
{
	float3 position : POSITION;
	float3 normal : NORMAL;
	float2 texCoord : TEXCOORD0;
...
//텍스쳐와 조명을 같이 사용하는 경우 정점 쉐이더의 출력을 위한 구조체이다.
struct VS_TRANSLATE_TEXTURED_LIGHTING_COLOR_OUTPUT
{
	float4 position : SV_POSITION;
	float3 positionW : POSITION;
	float3 normalW : NORMAL;
...
// 조명과 텍스쳐 매핑을 처리하는 정점 쉐이더 함수와 픽셀 쉐이더 함수를 다음과 같이 추가한다.
VS_TRANSLATE_TEXTURED_LIGHTING_COLOR_OUTPUT VSTranslatedTexturedLightingColor(VS__TRANSLATE_TEXTURED_LIGHTING_COLOR_INPUT input)
{
	VS_TRANSLATE_TEXTURED_LIGHTING_COLOR_OUTPUT output = (VS_TRANSLATE_TEXTURED_LIGHTING_COLOR_OUTPUT)0;
	output.normalW = mul(input.normal, (float3x3)gmtxWorld);
	output.positionW = mul(float4(input.position, 1.0f), gmtxWorld).xyz;
	output.position = mul(mul(float4(output.positionW, 1.0f), gmtxView), gmtxProjection);
...
float4 PSTranslatedTexturedLightingColor(VS_TRANSLATE_TEXTURED_LIGHTING_COLOR_OUTPUT input) : SV_Target
{
	input.normalW = normalize(input.normalW);
	float4 cIllumination = Lighting(input.positionW, input.normalW);
