...
//
CTranslatedTexturedIlluminatedShader::CTranslatedTexturedIlluminatedShader()
{
...
void CTranslatedTexturedIlluminatedShader::CreateShader(ID3D11Device* pd3dDevice)
{
	D3D11_INPUT_ELEMENT_DESC d3dInputElements[] =
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	...
	UINT nElements = ARRAYSIZE(d3dInputElements);
	CreateVertexShaderFromFile(pd3dDevice, L"Effect.fx", "VSTranslatedTexturedLightingColor", "vs_5_0", &m_pd3dVertexShader, d3dInputElements, nElements, &m_pd3dVertexLayout);
	CreatePixelShaderFromFile(pd3dDevice, L"Effect.fx", "PSTranslatedTexturedLightingColor", "ps_5_0", &m_pd3dPixelShader);
}
...
void CWaterBoxShader::BuildObjects(ID3D11Device* pd3dDevice, CHeightMapTerrain *pHeightMapTerrain)
{
	...
	int length = pHeightMapTerrain->GetLength();
	int width = pHeightMapTerrain->GetWidth();

	m_ppObjects[0]->SetPosition(width/2, 0.0f, length/2);
...
void CMirrorShader::BuildObjects(ID3D11Device *pd3dDevice, CHeightMapTerrain *pHeightMapTerrain)
{
	m_nObjects = 1;
	m_ppObjects = new CGameObject*[m_nObjects];
	...
	D3DX11CreateShaderResourceViewFromFile(pd3dDevice, _T("../../Assets/Image/Miscellaneous/color.jpg"), NULL, NULL, &pd3dsrvBaseTexture, NULL);
	pMirrorTexture->SetTexture(0, pd3dsrvBaseTexture);
	pMirrorTexture->SetSampler(0, pd3dBaseSamplerState);
