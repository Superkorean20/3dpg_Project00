...
void CWaterBoxShader::BuildObjects(ID3D11Device* pd3dDevice)
{
	...
	ZeroMemory(&d3dSamplerDesc, sizeof(D3D11_SAMPLER_DESC));
	d3dSamplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	d3dSamplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_MIRROR;
	d3dSamplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_MIRROR;
	d3dSamplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_MIRROR;
	d3dSamplerDesc.ComparisonFunc = D3D11_COMPARISON_NEVER;
	d3dSamplerDesc.MaxLOD = 0;
	d3dSamplerDesc.MinLOD = 0;
	pd3dDevice->CreateSamplerState(&d3dSamplerDesc, &pd3dBaseSamplerState);
